...
@Service
public class PayPointsBindingHandler implements ITextCommandHandler {
    ...
    public PayPointsBindingHandler(IResponseSender responseSender, IPaymentTypeService paymentTypeService,
    ...
    public void handle(Message message) {
        ...
        List<InlineButton> buttons = new java.util.ArrayList<>(paymentTypes
                ...
                .map(paymentType -> InlineButton.builder()
                        ...
                        .data(callbackDataService.buildData(CallbackQueryData.PAY_POINTS_BINDING, paymentType.getPid()))
                        .inlineType(InlineType.CALLBACK_DATA)
                        .build())
                .toList()
        );
        responseSender.sendMessage(chatId,
                "Выберите тип оплаты, к которому хотите выполнить привязку метода оплаты PayPoints. Типы оплаты отображены только для валюты \""
                        + FiatCurrency.RUB.getDisplayName() + "\".", keyboardBuildService.buildInline(buttons, 2));
    }

    @Override
    public TextCommand getTextCommand() {
        return TextCommand.PAY_POINTS_BINDING;
    }
}
