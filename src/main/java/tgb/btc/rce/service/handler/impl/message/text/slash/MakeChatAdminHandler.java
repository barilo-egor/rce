...
@Service
public class MakeChatAdminHandler implements ISlashCommandHandler {

    private final IChangeRoleService changeRoleService;

    public MakeChatAdminHandler(IChangeRoleService changeRoleService) {
        this.changeRoleService = changeRoleService;
    }

    @Override
    public void handle(Message message) {
        changeRoleService.changeRole(message, UserRole.CHAT_ADMIN);
    }

    @Override
    public SlashCommand getSlashCommand() {
        return SlashCommand.MAKE_CHAT_ADMIN;
    }
}
