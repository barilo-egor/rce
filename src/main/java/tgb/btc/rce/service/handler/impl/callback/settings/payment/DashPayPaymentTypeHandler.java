...
@Service
public class DashPayPaymentTypeHandler implements ICallbackQueryHandler {
    ...
    public DashPayPaymentTypeHandler(ICallbackDataService callbackDataService, IPaymentTypeService paymentTypeService,
    ...
    public void handle(CallbackQuery callbackQuery) {
        ...
        if (Objects.isNull(paymentType.getDashPayOrderMethod())) {
            responseSender.sendMessage(chatId, "Привязка типа оплаты <b>\"" + paymentType.getName()
                            + "\"</b> к методу оплаты DashPay отсутствует. Выберите новый метод.",
            ...
            responseSender.sendMessage(chatId, "Тип оплаты <b>\"" + paymentType.getName()
                            ...
                            + paymentType.getDashPayOrderMethod().getDisplayName() + "\"</b>.",
                    keyboardBuildService.buildInline(buttons, 2));
        }
    }
