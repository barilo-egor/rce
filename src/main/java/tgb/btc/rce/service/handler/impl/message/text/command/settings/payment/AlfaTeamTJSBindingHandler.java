...
public class AlfaTeamTJSBindingHandler implements ITextCommandHandler {
    ...
    public AlfaTeamTJSBindingHandler(IResponseSender responseSender, IPaymentTypeService paymentTypeService,
    ...
    public void handle(Message message) {
        ...
        List<InlineButton> buttons = new java.util.ArrayList<>(paymentTypes
                ...
                .map(paymentType -> InlineButton.builder()
                        ...
                        .data(callbackDataService.buildData(CallbackQueryData.ALFA_TEAM_TJS_PAYMENT_TYPE, paymentType.getPid(), false))
                        .inlineType(InlineType.CALLBACK_DATA)
                        .build())
                .toList()
        );
        Optional<PaymentType> optionalPaymentType = paymentTypes.stream()
                .filter(pt -> Objects.nonNull(pt.getAlfaTeamTJSPaymentOption()))
                .findFirst();
        String messageText;
        if (optionalPaymentType.isPresent()) {
            messageText = "Текущая привязка AlfaTeam TJS к типу оплаты <b>\"" + optionalPaymentType.get().getName() + "\"</b>.\n";
            buttons.add(InlineButton.builder()
                            .text("❌ Удалить привязку")
                            .data(callbackDataService.buildData(CallbackQueryData.ALFA_TEAM_TJS_PAYMENT_TYPE, optionalPaymentType.get().getPid(), true))
                    .build());
        } else {
            messageText = "";
        }
        messageText = messageText + "Выберите тип оплаты, к которому хотите выполнить привязку метода оплаты AlfaTeam. Типы оплаты отображены только для валюты \""
